        ...
        <el-card class="box-card" style="width:80%;margin:20px auto">
          <el-form ref="spuBaseForm" :model="spu" label-width="120px" :rules="spuBaseInfoRules">
            ...
            <el-form-item label="商品介绍" prop="decript">
              <el-upload
                  class="upload-demo"
                  :action="uploadDescUrl"
                  name="file"
                  :on-success="uploadDescSuccess"
...
export default {
  ...
  data() {
    return {
      uploadDescUrl: 'http://localhost:3000/api/product/spuinfodesc/spuInfoDescUpload',
      uploadImagesUrl: 'http://localhost:3000/api/product/spuimages/spuImagesUpload',
      fileList:[],
      fileDescList:[],
      catPathSub: null,
      brandIdSub: null,
  ...
  methods: {
        // 商品详情大图上传成功回调
      uploadDescSuccess(response){
        if (response.code >= 10000 && response.code < 20000){
          this.spu.decript.push(response.fullUrl)
        ...
        // 商品图集上传成功回调
      uploadImagesSuccess(response){
        if (response.code >= 10000 && response.code < 20000){
          this.spu.images.push(response.fullUrl)
          this.$message.success(response.msg)
        }else{
          this.$message.error(response.msg)
        }
      ...
      this.$http({
        ...
        .then(({ data }) => {
            if (data.code >= 10000 && data.code < 20000) {
              ...
              this.dataResp.memberLevels = data.memberLevelList.resultList;
            }else{
    ...
    generateSkus() {
      ...
      descartes.forEach((descar, descaridx) => {
        ...
        console.log(this.dataResp.memberLevels)
         console.log(this.dataResp.memberLevels.length)
    ...
    getShowSaleAttr() {
      ...
      if (!this.dataResp.steped[1]) {
        this.$http({
          ...
          if(data.code >= 10000 && data.code <20000){
            this.$message.success(data.msg);
            this.dataResp.saleAttrs = data.saleAttrList

            data.saleAttrList.forEach(item => {
        ...
        this.$http({
          ...
          if (data.code >= 10000 && data.code < 20000) {
            //先对表单的baseAttrs进行初始化
              ...
              data.baseAttrObjectList.forEach(item => {
              let attrArray = [];
            ...
            this.dataResp.attrGroups = data.baseAttrObjectList;
