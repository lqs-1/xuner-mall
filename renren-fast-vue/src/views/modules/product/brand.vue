  ...
  <div class="mod-config">
    ...
    <el-dialog title="关联分类" :visible.sync="cateRelationDialogVisible" width="30%">
      <el-popover placement="right-end" v-model="popCatelogSelectVisible">
        <category-cascader :catelogPath.sync="catelogIds"></category-cascader>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      addCatelogSelect () {
        ...
        this.$http({
          ...
          method: 'post',
          data: this.$http.adornData({brandId: this.brandId, catelogId: this.catelogIds[this.catelogIds.length - 1]}, false)
        }).then(({ data }) => {
          this.catelogIds = []
    ...
    getCateRelation () {
      this.$http({
        url: this.$http.adornUrl('/product/categorybrandrelation/relation/list'),
        method: 'get',
        params: this.$http.adornParams({
          ...
          if(data.code >=10000 && data.code < 20000){
            this.$message.success(data.msg)
            this.cateRelationTableData = data.categoryBrandRelationEntityList
          }else{
            this.$message.error(data.msg)
          }
      ...
      getDataList () {
        ...
        this.$http({
          ...
          if (data.code >= 10000 && data.code < 20000){
            this.$message.success(data.msg)
            this.dataList = data.page.resultList
