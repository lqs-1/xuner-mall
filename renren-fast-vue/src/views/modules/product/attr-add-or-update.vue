...
export default {
  data() {
    return {
      uploadUrl: 'http://localhost:3000/api/product/attr/attrIconUpload',
      fileList:[],
      visible: false,
      dataForm: {
        attrId: 0,
      ...
      if (path && path.length == 3) {
        this.$http({
          url: this.$http.adornUrl(
            `/product/attrgroup/list/nolimet/${path[path.length - 1]}`
          ),
          method: "get",
          params: this.$http.adornParams({ page: 1, limit: 10000000 })
           ...
           if (data && data.code >= 10000 && data.code < 20000) {
            ...
            this.attrGroups = data.attrGroupList;
          } else {
        ...
        if (this.dataForm.attrId) {
          this.$http({
            ...
            if (data.code >= 10000 && data.code < 20000) {
              ...
              //attrGroupId
              //catelogPath
              this.catelogPath = data.attr.catelogIds;
              this.$nextTick(() => {
